...
다음 표는 각 MSCP 레벨에서 에이전트 행동을 지배하는 핵심 수치 임계값을 요약합니다. 이 임계값들은 임의로 정해진 것이 아니며 — 각 값은 반복적 프로토타이핑에서 도출된 검증된 안정성 경계입니다.

| 파라미터 | 레벨 | 값 | 설명 |
|----------|:----:|:--:|------|
| 예측 오차 임계값 | L3+ | $0.30$ | 예측 오차가 이 값을 초과하면 행동 차단 |
| 정체성 델타 클램프 | L3+ | $0.05$ | 정체성 벡터의 사이클당 최대 변화량 |
| 수렴 엡실론 | L3+ | $0.05$ | 랴프노프 함수의 유계 증분 허용치 |
...
| Stage 2 | L3 | 예측 게이팅($\theta_{\text{pred}} = 0.30$, L3 §3.3)을 갖춘 **예측-비교-갱신을 통한 자기비판**. 자기모델이 비보정 상태가 되면 자신의 행동을 차단. |
